...
function deleteLineFromCart(event) {
  ...
  if (lineToDelete) {
    ...
    const productId = lineToDelete.dataset.productId;

    const newCart = cart.filter((cartItem) => cartItem.product.id !== Number(productId));

    calculateAndSetCartTotal(newCart);
  }
}


function calculateAndSetCartTotal(cart) {
  let total = 0;
  ...
  const totalElement = document.querySelector('.table tfoot .price');
  if (totalElement) {
